...
#if 0  /* Hash table support temporarily disabled */
...
class GeneratePass(ScopeTracker):
    ...
    def visit_AnnAssign(self, node):
        ...
        if tgt_type == 'str':
            if value.startswith("sds"):
                ...
                rvalue = f"sdsnew({value})"
            self.output(f'sdsfree({target}); {target} = {rvalue};\n', indent=True)
        else:
        ...
        if tgt_type == 'str':
            if len(tgt_names) == 1:
                if value.startswith("sds"):
                    ...
                    rvalue = f"sdsnew({value})"
                self.output(f'sdsfree({tgt_s}); {tgt_s} = {rvalue};\n', indent=True)
            else:
                raise self.exception(f"Assigning strings to multiple targets is not supported.")
        else:
    ...
    def visit_Constant(self, node):
        const_type = self.syms.get_type_from_value(node.value)
        if const_type == 'str':
            return f'"{node.value}"'
        else:
            return node.value
    ...
    def visit_BinOp(self, node):
        ...
        l_type = self.syms.get_type_from_value(node.left)
        r_type = self.syms.get_type_from_value(node.right)
        if l_type == 'str' or r_type == 'str':
            if l_type != r_type:
                raise self.exception(f'Doing {op_s} on {l_type} and {r_type} is not supported.')
            if isinstance(node.op, ast.Add):
                return f'sdscat({left}, {right})'
            else:
                raise self.exception(f'String {op_s} operation not supported.')
